...
public class VirusTree {
    ...
    public VirusTree insert(Person c, Person p) throws IllegalArgumentException {
        ...
        if (getNode(c) != null || getNode(p) == null || c == null || p == null)
            throw new IllegalArgumentException("Can't insert in VirusTree");

        VirusTree vt= getNode(p);
        vt.children.add(new VirusTree(c));
        return getNode(c);
    ...
    public List<Person> virusRouteTo(Person c) {
        ...
        for (VirusTree ch : children) {
            LinkedList<Person> k= (LinkedList) ch.virusRouteTo(c);
            if (k != null) {
                k.addFirst(root);
                return k;
            }
    ...
    public Person commonAncestor(Person child1, Person child2) {
        ...
        List<Person> l1= virusRouteTo(child1);
        List<Person> l2= virusRouteTo(child2);
    ...
    public boolean equals(Object ob) {
        ...
        if (ob.getClass() != VirusTree.class) return false;
        VirusTree obt= (VirusTree) ob; // cast to obt - object tree
        if (root != obt.getRoot() || childrenSize() != obt.childrenSize()) { return false; }
        for (VirusTree dt1 : children) {
            boolean existsInOb= help(dt1, obt.children);
            if (!existsInOb) return false;
        }
        return true;
    ...
    /** Return true iff st equals some member of s2 */
    public boolean help(VirusTree st, Set<VirusTree> s2) {
        for (VirusTree dt2 : s2) {
            if (dt2.equals(st)) return true;
        }
        return false;
    }
